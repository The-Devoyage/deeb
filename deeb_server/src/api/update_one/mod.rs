...
pub async fn update_one(
    ...
    let record = database.deeb.find_one::<Value>(&entity, query.clone(), None).await;
    ...
    if record.is_none() {
        return Response::new(StatusCode::NOT_FOUND).message("Failed to find document");
    }
    ...
    match allowed {
        ...
        Err(e) => {
            log::error!("{:?}", e);
            return Response::new(StatusCode::INTERNAL_SERVER_ERROR)
                .message("Something went wrong when checking rule access.");
        }
    }
}
